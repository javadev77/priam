...
@Lazy
@Transactional
public interface FichierCPDao extends JpaRepository<FichierCP, Long> {
    ...
    @Query("SELECT DISTINCT new fr.sacem.priam.model.domain.dto.FileDto(f.id, f.nomFichier, fam.code, typu.code, f.dateDebutChargt, f.dateFinChargt, f.nbLignes, f.statut) " +
            "FROM FichierCP AS f LEFT JOIN f.famille AS fam LEFT JOIN f.typeUtilisation AS typu " +
    ...
    @Transactional(readOnly = true)
    @Query("SELECT DISTINCT new fr.sacem.priam.model.domain.dto.FileDto(f.id, f.nomFichier, fam.code, typu.code, f.dateDebutChargt, f.dateFinChargt, f.nbLignes, f.statut) " +
            "FROM FichierCP AS f JOIN f.famille AS fam JOIN f.typeUtilisation AS typu " +
    ...
    @Query("UPDATE FichierCP f SET f.statut= :statut WHERE f.id = :id")
    ...
    @Query("SELECT f " +
            ...
            "AND f.statut=:status")
    List<FichierCP> findFichiersByIdProgramme(@Param("numProg") String numProg, @Param("status") Status status);
    
    
    @Transactional
    @Modifying(clearAutomatically = true)
    @Query("UPDATE FichierCP f SET f.programme.numProg = NULL, f.statut =:status WHERE f.programme.numProg = :numProg")
    void clearSelectedFichiers(@Param("numProg") String numProg,@Param("status") Status status);
    
    
    @Transactional
    @Modifying(clearAutomatically = true)
    @Query("UPDATE FichierCP f SET f.programme.numProg = :numProg, f.statut =:status  WHERE f.id IN (:idFichiers) ")
    ...
    FichierCP findByNomFichier(String s);
