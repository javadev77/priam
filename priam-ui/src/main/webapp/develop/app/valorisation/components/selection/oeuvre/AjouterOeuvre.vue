<template>
  <div style="overflow-y: auto; max-height: 730px;">
    ...
    <modal v-if="showPopup">
        ...
        <template slot="body" v-if="programme.typeUtilisation === 'FD12'">
          <div style="text-align: justify" v-html="messageFD12">
          </div>
        </template>
        <template  slot="body" v-if="programme.typeUtilisation === 'FD06'">
          <div style="text-align: justify" v-html="messageFD06">
          </div>
        </template>
      ...
      created() {
        const customActions = {
          findLigneProgrammeByProgramme: {
            method: 'POST',
            url: process.env.CONTEXT_ROOT_PRIAM_FV + 'app/rest/ligneProgramme/search'
          },
            ...
            this.resource.findLigneProgrammeByProgramme({ide12 : oeuvre.ide12, numProg : this.programme.numProg})
              ...
              .then(data => {
                  if(data && data.content.length > 0 ) {
                      ...
                      this.showPopup = true;
                      if(this.programme.typeUtilisation === 'FD12') {
                          this.messageFD12 = 'Attention, cette oeuvre est déjà présente au niveau du programme avec Points = '
                            + ligneProg.pointsMontant + ', êtes-vous sûr de vouloir la remplacer ?';

                      } else if(this.programme.typeUtilisation === 'FD06') {
                          this.messageFD06 = 'Attention, cette oeuvre est déjà présente au niveau du programme avec Points '
                            + ligneProg.pointsMontant + ', êtes-vous sûr de vouloir la remplacer ?';
                      }
                  } else {
